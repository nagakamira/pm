...
for i in $@; do
    case "$i" in
        -h|--help)
            echo "usage: `basename $0` [operation] [arguments] [option]"
            echo "operation:"
            echo "  -a                add package(s)"
            echo "  -A                add group of packages"
            echo "  -b                build package(s)"
            echo "  -B                build group of packages"
            echo "  -c                show conflicting files"
            echo "  -d                delete package(s)"
            echo "  -D                delete group of packages"
            echo "  -g                show group of packages"
            echo "  -G                show all the groups"
            echo "  -i                show package information"
            echo "  -l                show package filelist"
            echo "  -m                add build dependencies"
            echo "  -o                show the file ownership"
            echo "  -s                generate SHA hash"
            echo "  -u                update package(s)"
            echo "  -U                update all the packages"
            echo "options:"
            echo "  reinstall         force install package"
            echo "  rootdir=<path>    change root directory"
